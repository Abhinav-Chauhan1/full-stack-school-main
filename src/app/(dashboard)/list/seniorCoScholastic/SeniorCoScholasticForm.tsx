...
import { 
  saveSeniorCoScholastic, 
  checkExistingSeniorCoScholastic,
  getSeniorCoScholasticData,
  getInitialData 
} from "./actions";

// Define schema for validation
const seniorCoScholasticSchema = z.object({
  marks: z.array(z.object({
    seniorMarkId: z.number(),
...
type SeniorCoScholasticSchema = z.infer<typeof seniorCoScholasticSchema>;

type SeniorCoScholasticFormProps = {
  type: "create" | "update";
  relatedData?: any;
  setOpen: Dispatch<SetStateAction<boolean>>;
};

const SeniorCoScholasticForm = ({ type = "create", relatedData, setOpen }: SeniorCoScholasticFormProps) => {
  ...
  const [students, setStudents] = useState<Array<{ id: string, seniorMarkId: number, name: string, admissionno: string }>>([]);
  ...
  } = useForm<SeniorCoScholasticSchema>({
    resolver: zodResolver(seniorCoScholasticSchema),
    defaultValues: {
      marks: []
    }
  });
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        // Get students data
        const studentsData = await getSeniorCoScholasticData({
        ...
        if (studentsData.success && studentsData.students) {
          ...
          const existingDataResult = await checkExistingSeniorCoScholastic({
            classId: selectedClass,
            sectionId: selectedSection,
            sessionId: selectedSession
          });
          ...
          if (existingDataResult.success && existingDataResult.data && existingDataResult.data.length > 0) {
            ...
            const formValues = {
              marks: formattedStudents.map(student => {
                // Find existing record for this student by seniorMarkId
                const existingRecord = existingDataResult.data.find(
                  (record: any) => record.seniorMarkId === student.seniorMarkId
                );
                
                return {
                  seniorMarkId: student.seniorMarkId,
  ...
  const onSubmit = async (data: SeniorCoScholasticSchema) => {
    try {
      const result = await saveSeniorCoScholastic({
        marks: data.marks,
        term
      });
                ...
                {students.map((student, index) => (
                  <tr key={student.id} className="even:bg-gray-50">
                    <td className="p-2 border">
                      <input 
                        type="hidden" 
                        {...register(`marks.${index}.seniorMarkId`)} 
                        value={student.seniorMarkId}
                      />
...
export default SeniorCoScholasticForm;
