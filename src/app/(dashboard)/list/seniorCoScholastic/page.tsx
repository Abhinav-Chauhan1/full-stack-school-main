...
/**
 * SeniorCoScholasticPage component
 * @param {Object} props
 * @param {Object} props.searchParams - URL search parameters
 */
const SeniorCoScholasticPage = async ({
  searchParams,
}: {
  ...
  const classes = await prisma.class.findMany({
    where: {
      AND: [
        {
          classNumber: {
            gte: 9, // Senior classes only (9-10)
            lte: 10
          }
        },
  ...
  if (classId) {
    query.seniorMark = {
      ...query.seniorMark,
      sectionSubject: {
        ...query.seniorMark?.sectionSubject,
        section: {
          ...query.seniorMark?.sectionSubject?.section,
  ...
  const [data, count] = await prisma.$transaction([
    prisma.seniorCoScholastic.findMany({
      where: query,
      include: {
        seniorMark: {
          include: {
      ...
      orderBy: [
        { seniorMark: { student: { name: "asc" } } }
      ],
      take: ITEM_PER_PAGE,
      skip: ITEM_PER_PAGE * (p - 1),
    }),
    prisma.seniorCoScholastic.count({ where: query }),
  ]);
  ...
  const getColumns = (term: string = 'TERM1') => {
    const baseColumns = [
      {
        header: "Student Name",
        accessor: "seniorMark.student.name",
      },
      {
        header: "Admission No",
        accessor: "seniorMark.student.admissionno",
      },
    ];
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h1 className="hidden md:block text-lg font-semibold">
            Senior Co-Scholastic Activities {role === "teacher" && `- ${assignedClassStr} ${assignedSectionStr}`}
          </h1>
          <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <div className="flex items-center gap-4 self-end">
              ...
              {(role === "admin" || role === "teacher") && (
                <FormContainer table="seniorCoScholastic" type="create" />
              )}
            </div>
          </div>
        </div>
          ...
          data.length > 0 ? (
            <Table
              ...
              renderRow={(item) => (
                ...
                >
                  <td className="p-4">{item.seniorMark?.student?.name || '-'}</td>
                  <td className="p-4">{item.seniorMark?.student?.admissionno || '-'}</td>
                  {termFilter === 'TERM1' ? (
                    <>
...
export default SeniorCoScholasticPage;
